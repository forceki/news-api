...
async def _create_topic(client: AsyncClient, headers: dict, name: str) -> int:
    resp = await client.post("/topics", json={"name": name}, headers=headers)
...
class TestCreateNews:
    ...
    async def test_create_news_with_topics(self, client: AsyncClient, auth_token):
        topic_id = await _create_topic(client, auth_token, "Tech")
    ...
    async def test_create_news_auto_slug(self, client: AsyncClient, auth_token):
        response = await client.post("/news", json={
            "title": "Hello World From FastAPI",
            "content": "Auto slug content.",
        }, headers=auth_token)
        assert response.status_code == 201
        assert response.json()["data"]["slug"] == "hello-world-from-fastapi"
    ...
    async def test_create_news_duplicate_title(self, client: AsyncClient, auth_token):
...
class TestUpdateNews:
    async def test_update_news_success(self, client: AsyncClient, auth_token):
        ...
        assert response.json()["data"]["slug"] == "updated-title"

    async def test_update_news_change_topics(self, client: AsyncClient, auth_token):
        topic_id = await _create_topic(client, auth_token, "Sports")
        create_resp = await client.post("/news", json={
